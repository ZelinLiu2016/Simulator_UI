...
class Estimator(object):
    ...
    def predict(self, s, a=None):
        ...
        if a:
            ...
            return [self.models[m].predict([s])[0] for m in range(len(self.env.action_space))]
...
class Env:
    def __init__(self, days):
        self.action_space = [-20, -15, -10, -5, 0, 5, 10, 15, 20]
        self.day = 0
        self.max_day = days
        self.yes_pre = 0
        self.yes_real = 0
    ...
    def step(self, pxdiff):
        ...
        order_diff = min(max(self.yes_real - self.yes_pre, -150), 150)
        self.yes_pre = n_orders
        self.yes_real = orders
        return [self.day, order_diff, n_orders], orders*(150 + pxdiff), is_done, {}
...
def q_learning_with_value_approximation(env, estm, epoch_num, discount_factor=0.9, epsilon=0.1, epsilon_decay=0.99):
    for i_epoch_num in range(epoch_num):
        ...
        for it in itertools.count():
            ...
            max_action = int(np.where(action_probs == np.max(action_probs))[0].tolist()[-1])
...
def generate_q_table(esm, env, e):
    ...
    f = open('qtable_3d_e_%.2f_fixrevenue_orderdiff.csv' % e, 'w')
...
def to_pickle(esm, env, e, r):
    f = open('qtable_3d_e_%.2f_fixrevenue_orderdiff_150_%d.pkl' % (e, r), 'w')
    pickle.dump([esm, env], f)
    f.close()
...
if __name__ == '__main__':
    ...
    eps = 0.95
    rnd = 1000
    q_learning_with_value_approximation(environment, estimator, rnd, epsilon=eps)
    to_pickle(estimator, environment, eps, rnd)
